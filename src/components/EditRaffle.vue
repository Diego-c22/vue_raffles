    ...
    <div class="bg-white rounded-md shadow-2xl absolute border top-5 right-10 left-10 z-30 p-2 bg-opacity-90">
      ...
      <div class="">
        <form enctype="multipart/form-data" @submit.prevent="createRaffle" class="flex flex-col items-center w-full">
          ...
          <button type="submit" class="button bg-blue-600 hover:bg-blue-700 p-1 w-1/4">Guardar</button>
        </form>
      </div>
    </div>
  </transition>
...
export default {
  ...
  mounted () {
    this.name = this.raffle.name
    this.description = this.raffle.description
    this.prizePrice = this.raffle.prize_price
    this.ticketPrice = this.raffle.ticket_price
    this.drawingDate = this.raffle.drawing_date
    this.raffleEnd = this.raffle.raffle_end
    this.raffleActivation = this.raffle.raffle_activation
    this.numbersQuantity = this.raffle.numbers_quantity
    this.mainRaffle = this.raffle.main
  },
  ...
  methods: {
    ...
    sendData (fd) {
      editRaffle(this.raffle.id, fd, this.token)
  ...
  computed: {
    raffle () {
      return this.$store.state.raffle
    }
  }
}
</script>
